...
export const metadata = {
  title: "Compare layouts | One of One Cleaning Solutions",
};
      ...
      <div className="w-full max-w-5xl">
        <header className="text-center">
          ...
          <p className="mt-3 text-zinc-600">
            Three landing-page directions. <strong>Bold &amp; Edgy</strong> is the
            current main site —{" "}
            <Link href="/" className="font-medium text-zinc-900 underline underline-offset-2">
              view the live site →
            </Link>
            <br className="hidden sm:block" />
            The other two are kept here for reference.
          </p>
        </header>
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-3">
          {options.map((o) => (
            <Link
              key={o.href}
              href={o.href}
              className={`group relative flex flex-col rounded-2xl border p-6 shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl ${o.cardClass}`}
            >
              {o.live && (
                <span className="absolute right-4 top-4 rounded-full bg-[#e6332a] px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-white">
                  ★ Live
                </span>
              )}
